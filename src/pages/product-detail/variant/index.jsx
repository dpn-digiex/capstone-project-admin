...
const VariantModal = ({ showModal = false, setShowModal = () => {}, item, onSetVariants = () => {} }) => {
  ...
  const handleAddOption = () => {
    ...
    setOption({ ...INIT_OPTION, _id: Date.now() })
  }
  const handleRemoveOption = (optionId) => {
    setVariant((prev) => ({ ...prev, options: prev.options.filter((opt) => opt._id !== optionId) }))
  }
          ...
          <div className='flex flex-col gap-4 bg-white px-6 py-4 rounded-xl w-2/3 max-h-[75vh] overflow-auto'>
                ...
                <div className='flex flex-col gap-2 mt-4'>
                  {variant.options?.map((option) => (
                    <div key={option._id} className='w-full p-2 rounded bg-gray-50 text-sm flex items-center gap-4'>
                      ...
                      <button type='button' onClick={() => handleRemoveOption(option._id)}>
            ...
            <div className='flex items-center justify-center gap-4'>
              ...
              <button
                ...
                onClick={() => {
                  onSetVariants((prev) =>
                    prev.filter((_v) => _v._id !== variant._id).concat({ ...variant, _id: variant._id ?? Date.now() })
                  )
                  setShowModal(false)
                }}
              >
